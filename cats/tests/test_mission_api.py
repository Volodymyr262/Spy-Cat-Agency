...
def test_assign_cat_to_existing_mission(api_client):
    """
    Test assigning a free cat via custom endpoint.
    """
    mission = MissionFactory(cat=None)
    free_cat = SpyCatFactory()

    url = reverse("mission-assign-cat", args=[mission.id])
    payload = {"cat_id": free_cat.id}

    response = api_client.post(url, payload, format="json")  # Тепер це POST
...
def test_cannot_assign_busy_cat_to_mission(api_client):
    """
    Test that we cannot assign a BUSY cat via custom endpoint.
    ...
    url = reverse("mission-assign-cat", args=[mission_b.id])
    payload = {"cat_id": busy_cat.id}

    response = api_client.post(url, payload, format="json")

    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert "already assigned" in str(response.data)
